...
void BlockDataHandler(UNIT * unit, char * text, int32_t offset, MODE mode, int16_t etsModeSet)
{
	...
	{	printf("%i",sampleCount);
		status = ps2000aGetValues(unit->handle, 0, (uint32_t*) &sampleCount, 10, ratioMode, 0, NULL); //changed by D.S.
		printf("%i",sampleCount);
...
void ORAnalogueDigitalTriggered(UNIT * unit)
{
	...
	int16_t	triggerVoltage = mv_to_adc(0, unit->channelSettings[PS2000A_CHANNEL_A].range, unit);//changed
	printf("%i",triggerVoltage);
	PS2000A_TRIGGER_CHANNEL_PROPERTIES sourceDetails = {	triggerVoltage,		// thresholdUpper
															256 * 10,			// thresholdUpper Hysteresis
															triggerVoltage,		// thresholdLower
															256 * 10,			// thresholdLower Hysteresis
	...
	conditions[0].channelA				= PS2000A_CONDITION_TRUE;		/*changed*/			// Channel A
...
int32_t main(int argc, char *argv[])
{
	...
	uint32_t timebase = 1;
		...
		retrievedSamples = NoSamples;
		status = ps2000aGetValues((unit.handle),0,&retrievedSamples,PS2000A_RATIO_MODE_NONE,0,0,NULL);
